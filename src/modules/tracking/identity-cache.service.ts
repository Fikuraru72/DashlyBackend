...
export class IdentityCacheService {
  ...
  async resolveParticipantId(
    ...
    // 1. Check Redis cache
    const cached = await this.redisService.getCachedParticipant(
      cacheKey as any,
    );
    if (cached !== null) {
      return cached;
    ...
    const participant = await this.db.query.eventParticipants.findFirst({
      where: (table, { and, eq }) =>
        and(eq(table.eventId, eventId), eq(table.userId, userId)),
    });

    if (!participant) {
      this.logger.warn(
        `Participant not found in DB for Event ${eventId}, User ${userId}`,
      );
      return null;
    }

    // 3. Populate cache for next time
    await this.redisService.setCachedParticipant(
      cacheKey as any,
      participant.id,
    );
